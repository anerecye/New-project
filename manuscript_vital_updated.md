...
We then performed a larger independent cross-disease portability validation using 3,000 randomly sampled ClinVar P/LP variants outside the arrhythmia development genes and outside the previously used cardiomyopathy, epilepsy, and hearing-loss external panels. The sample had two prespecified strata. First, 2,000 variants were sampled from the top 100 genes by number of ClinVar P/LP submissions after exclusions; this top-100 definition was based only on ClinVar P/LP count, not on clinical importance. Second, 1,000 variants were sampled from the remaining non-arrhythmia, non-external-panel ClinVar P/LP pool. The same design was repeated for the archived January 2023 ClinVar snapshot and compared with the April 21, 2026 current snapshot for historical endpoint analysis. Sanity checks explicitly tested for arrhythmia gene leakage, overlap with the previous external panels, duplicate variation IDs, and duplicate variant keys. These cross-disease analyses were treated as portability and false-positive-burden validation. The current 2026 sample does not contain independent truth labels; the 2023-to-2026 historical sample provides independent strict and expanded endpoints, but those endpoints remain sparse and heterogeneous across disease mechanisms.
...
The larger current cross-disease validation sample contained 3,000 unique ClinVar P/LP variants from 726 genes after excluding all 20 arrhythmia genes and all genes used in the earlier cardiomyopathy, epilepsy, and hearing-loss stress-test panels. The enriched stratum contained 2,000 variants sampled from the top 100 genes by number of ClinVar P/LP submissions after these exclusions; the fully random stratum contained 1,000 variants from the remaining eligible ClinVar P/LP pool. Sanity checks confirmed 0 arrhythmia-gene leaks, 0 previous external-panel overlaps, and 0 duplicate variant IDs or variant keys after sampling.

Frequency evidence was observed for 668/3,000 variants (22.3%; 95% CI 20.8%-23.8%). A naive popmax/global AF >1e-5 screen flagged 332/3,000 variants (11.1%; 95% CI 10.0%-12.2%). Requiring AC-supported frequency evidence reduced this to 71/3,000 (2.37%; 95% CI 1.88%-2.97%). VITAL red reduced the urgent review queue to 3/3,000 variants (0.10%; 95% CI 0.034%-0.294%), equivalent to an expected 10.0 red calls per 10,000 random ClinVar P/LP variants (95% CI 3.4-29.4) and a 110.7-fold compression relative to naive AF screening. The red calls were PRG4 VCV003574703, MNS1 VCV003024123, and DYNC2H1 VCV002749617; each combined AC-supported frequency tension with weak review status rather than representing frequency alone. These variants were not adjudicated as false positives or false pathogenic assertions in this analysis; they define a compact manual-review set requiring disease-mechanism, inheritance, phenotype, and literature review. This is the desired clinical behavior for a reclassification risk prioritization framework: it creates a very small human-review queue rather than converting hundreds of population-frequency alerts into urgent work.

The same sampling design applied to the January 2023 ClinVar snapshot produced 3,000 unique baseline P/LP variants, again with 0 arrhythmia leaks, 0 previous external-panel overlaps, and 0 duplicate variant IDs or keys. Baseline VITAL red flagged 13/3,000 variants (0.43%; 95% CI 0.25%-0.74%), compared with 379/3,000 naive AF flags (12.6%; 95% CI 11.5%-13.9%). Against the April 21, 2026 snapshot, 23/3,000 variants met the strict endpoint of P/LP to B/LB or VUS. VITAL red captured 1/23 strict events (recall 4.3%; 95% CI 0.8%-21.0%) with 1/13 red precision (7.7%; 95% CI 1.4%-33.3%), 99.6% specificity, and a false-positive rate of 0.40%. Thus, strict reclassification remains rare and VITAL should not be presented as a broad predictor of future downgrades.

For the expanded instability endpoint (P/LP to non-P/LP, clinical-significance change, or review-status change), 794/3,000 baseline variants changed by 2026. VITAL red contained 11/13 expanded events (precision 84.6%; 95% CI 57.8%-95.7%) and achieved 99.9% specificity (95% CI 99.7%-99.98%), but recall remained low at 1.4% (95% CI 0.8%-2.5%). This confirms the main operating profile: VITAL red is a high-specificity, low-recall review-priority layer. It enriches for records with future instability under broader audit endpoints, but it does not capture most future ClinVar changes and should not be used as a stand-alone prediction model.
...
Eighth, the historical analysis is deliberately secondary. It is retained in the main text to document the audit trail, alignment checks, and negative evidence against overclaiming, not to give the impression of a powered predictive validation. In arrhythmia alone, only 2 baseline 2023 variants were VITAL red, and 1/2 later reached the broad instability endpoint, compared with 113/1,669 overall. In the earlier pooled multi-domain analysis, n increased to 3,063 but red calls remained sparse. The larger independent 3,000-variant 2023 cross-disease sample made the tradeoff clearer: VITAL red captured only 1/23 strict P/LP-to-B/LB-or-VUS events, but it retained 99.6% specificity and showed strong enrichment for the expanded instability endpoint (11/13 red events). This is not broad prediction. It is an extreme-priority monitoring signal that sacrifices recall to keep the review queue short.

Ninth, cross-disease validation argues against simple arrhythmia overfitting while preserving an appropriately conservative interpretation. In the current independent 3,000-variant sample outside arrhythmia and previous external-panel genes, naive AF screening produced 332 flags, whereas VITAL red produced 3. The three red calls require case-level review, but their existence is useful: the model does not become suspiciously silent outside arrhythmia, and it also does not create a large red-priority queue. The 2023 cross-disease sample showed the same pattern at a different freeze date: 379 naive AF flags versus 13 VITAL-red calls. These results strengthen the claim that VITAL reduces false-positive re-evaluation burden across heterogeneous ClinVar P/LP records. They do not prove disease-agnostic sensitivity, because recessive carrier states, founder variants, incomplete penetrance, mitochondrial mechanisms, and disease-specific thresholds may require inheritance-aware calibration.
...
- `data/processed/vital_cross_disease_3000_cross_disease_validation_summary.csv`: independent current 3,000-variant cross-disease portability summary with Wilson confidence intervals.
- `data/processed/vital_cross_disease_3000_cross_disease_outliers.csv`: current cross-disease VITAL-red and high-score outliers for manual review.
