# Severe Annotation Does Not Shield ClinVar P/LP Arrhythmia Variants from Frequency Tension: VITAL Prioritizes Re-Review

## Abstract

**Purpose:** Up to 35% of ClinVar P/LP LOF/splice arrhythmia variants with observed population data show population-frequency contradiction under ancestry-aware analysis. This is a clinical misinterpretation hazard, because global-AF-only workflows can miss ancestry-specific frequency tension while severe annotation can falsely reassure reviewers. We evaluated this guideline implementation gap and developed VITAL (Variant Interpretation Tension and Allele Load), an explainable review-prioritization framework for clinical laboratories with bioinformatics support.

**Methods:** We collapsed ClinVar P/LP records across 20 arrhythmia genes to 1,731 unique variants and cross-referenced them with gnomAD v4.1.1 exome and ancestry-specific data. VITAL explicitly models exact allele matches, allele discordance, no-record states, global AF, popmax AF, AC, variant type, technical detectability, gene context, and review fragility. It was evaluated as a human-review workflow, not as an automated classifier. Component weights were prespecified to preserve interpretability and stable behavior under sparse-event conditions; a restricted frequency-positive calibration audit checked design consistency but did not refit the score. Red-priority cases were further examined with real gnomAD v4.1 LOEUF values, Human Protein Atlas v23 heart-muscle expression, and mechanism-specific case interpretation.

**Results:** Only 350/1,731 variants (20.2%) had exact gnomAD allele matches; 334 had usable AF data. Global AF alone detected 13 variants above 1e-5, whereas popmax/global screening detected 115, including 102 globally rare but population-enriched variants. Indels were enriched in the non-overlap set (47.2% vs 28.9%; OR=2.20; BH q=1.08e-9), showing that absence from gnomAD is not equivalent to rarity for structurally complex variants. The strongest biological pattern was severe-annotation frequency discordance: 92/262 AF-observed LOF/splice assertions (35.1%) had popmax/global AF >1e-5, similar to missense assertions (19/66, 28.8%), and the signal persisted after excluding the recessive/biallelic arrhythmia genes CASQ2 and TRDN (57/197, 28.9%). Mechanism triage showed that this is a tension universe, not an error-rate estimate: 35/92 severe-discordant assertions were in carrier-compatible CASQ2/TRDN contexts, while only 1/57 non-recessive severe-discordant assertions was AC-supported. A naive popmax/global AF >1e-5 screen flagged 115 variants; AC-supported screening flagged 9; the red-priority queue contained 3, a 97.4% reduction in actionable calls. These cases were biologically distinct: SCN5A as a high-frequency haplotype/drug-response susceptibility assertion, TRDN as a high-expression but LOEUF-tolerant recessive-carrier signal, and KCNH2 as a borderline splice assertion in a constrained dominant LQTS gene. In an independent 3,000-variant cross-disease burden audit, naive AF screening flagged 332 variants (11.1%; 95% CI 10.0%-12.2%), whereas the red-priority queue contained 3 (0.10%; 95% CI 0.034%-0.294%). A real reclassification audit identified 23 P/LP-to-VUS/B/LB events from 2023 to 2026; VITAL-red captured one frequency-driven weak-review downgrade (CFAP91 VCV000812096), while one expert-panel curated downgrade (RYR1 VCV001214001) had no frequency tension and was correctly outside VITAL's scope. Historical audits confirmed the operating profile: high specificity, low recall, and usefulness for review-burden compression rather than broad prediction.

**Conclusion:** Severe annotation alone is not sufficient to override population-frequency contradiction, and global-AF-only pipelines are insufficient for frequency-based review prioritization. VITAL turns hidden frequency-assertion tension into a short, explainable laboratory review queue while preserving gray no-frequency-evidence states and leaving final classification to expert clinical interpretation.
...
Although population frequency can provide benign evidence within the ACMG/AMP framework, frequency observations do not by themselves resolve disease mechanism, penetrance, allelic phase, technical representation, or the evidentiary strength of the underlying clinical assertion. Recent ClinGen recommendations have further clarified that rarity and functional evidence should be applied in a disease-aware and evidence-calibrated manner rather than as isolated rules. The gap is not the existence of ACMG/AMP frequency criteria; the gap is operational implementation. Global-AF-only and absence-as-rarity workflows can systematically miss or misread the very frequency evidence that BS1/BA1-style review is meant to capture.
...
For real classification-level clinical impact, we separately audited strict future reclassification events in the independent 3,000-variant cross-disease historical set. This audit used public ClinVar category changes as the endpoint: P/LP in January 2023 and VUS, B/LB, or no longer P/LP by April 2026. We reported the number of real strict events, how many were captured by VITAL red-priority, and case-level examples. Because public ClinVar does not contain patient-level outcomes, this is a classification-impact audit rather than a patient-case or downstream-care audit.

We also used current expert-panel review status among strict future downgrades as an external curated truth stress-test. The purpose was not to prove that VITAL predicts expert-panel downgrades; it was to ask whether a curated downgrade without population-frequency tension would be inappropriately captured by a frequency-assertion tool. This is a deliberately hard negative-control for scope: VITAL should not flag all curated reclassifications, only those under frequency-assertion tension.

### Score interpretation, weight-consistency audit, and AC-gate audit
...
Because unrestricted endpoint calibration conflated general ClinVar churn with frequency-assertion discordance, all weight-consistency analyses were restricted to the frequency-positive universe. Within variants with observed AF >1e-5, we used three non-negative logistic probes for the expanded instability endpoint: review-only features (review fragility, single-submitter status, weak review), frequency-tension features (AF pressure, AC reliability, popmax enrichment, variant-type tension, technical detectability, and gene constraint), and the combined review-plus-tension feature set. These probes were evaluated by 5-fold out-of-fold prediction and bootstrapped confidence intervals, but they were not used to optimize, select, or replace VITAL weights. The purpose was to test whether the prespecified design structure was consistent with observed frequency-positive instability signals.
...
### Severe-annotation discordance and expert-panel comparator

To test whether frequency tension was confined to benign-looking missense variation or extended into variants normally treated as severe by annotation, we defined a severe-annotation class comprising LOF and splice/intronic records. We compared naive frequency tension (global or popmax AF >1e-5), AC-supported frequency tension, and red-priority calls across severe annotations, missense annotations, and LOF subtypes. We repeated the severe-annotation summary after excluding CASQ2 and TRDN, the canonical recessive/biallelic arrhythmia genes, and within genes with external gnomAD LOEUF upper confidence bound <0.5 or <0.6 where constraint metrics were available. These severe-annotation tests were descriptive biological audits and were not used to compute VITAL scores.

To avoid conflating frequency discordance with interpretation error, we also performed mechanism triage of severe-discordant assertions. Categories were deliberately conservative: carrier-compatible recessive/biallelic architecture (CASQ2/TRDN), non-recessive AC-supported high-penetrance tension, non-recessive constrained-gene low-AC surveillance, and non-recessive architecture-unresolved surveillance. This triage was not a clinical adjudication of penetrance, founder effect, or pathogenicity; it was a way to separate biologically plausible explanations from unresolved frequency-assertion tension.

As an external curated comparator independent of ClinVar label churn, we evaluated expert-panel reviewed P/LP assertions in the 3,000-variant cross-disease set. Expert-panel records were not treated as automatic truth labels for downgrade. They served as a specificity and stability comparator asking whether VITAL would incorrectly promote curated high-frequency exceptions into the urgent red-priority queue.
...
Global AF alone identified only 13 arrhythmia P/LP variants with AF >1e-5 and 1 with AF >1e-4. In contrast, popmax/global screening identified 115 variants with AF >1e-5 and 9 with AF >1e-4. Thus, a global-AF-only ACMG-style screen would miss 102/115 (88.7%) ancestry-aware frequency alerts detected by popmax/global analysis. Among 328 variants with complete popmax data, those same 102 variants (31.1%) were globally rare (global AF <=1e-5) but population-enriched (popmax AF >1e-5). This is not a marginal edge case: global AF compresses ancestry-specific signals strongly enough to hide most frequency contradictions detected by ancestry-aware screening.

The central message is therefore simple: frequency tension is not rare; it is systematically hidden by global-AF-only workflows and by the tendency to treat missing frequency evidence as absence. Global-AF-only pipelines are insufficient for clinical database auditing because they can leave a high-frequency ancestry-specific P/LP assertion looking artificially rare. VITAL does not make this tension disappear. It makes it small enough to inspect.
...
### Severe annotations do not protect against frequency discordance

The central biological claim is blunt: up to 35% of ClinVar P/LP LOF/splice variants with observed population data show population-frequency contradiction under ancestry-aware analysis. Among 334 AF-observed arrhythmia P/LP variants, 115 (34.4%) had popmax/global AF >1e-5. This was not confined to benign-looking missense assertions: 92/262 LOF/splice records (35.1%) were frequency-discordant, similar to missense records (19/66, 28.8%; Fisher OR=1.34, p=0.384). Within LOF subtypes, frequency discordance was present in 29/82 frameshift variants (35.4%), 33/108 stop-gained variants (30.6%), and 23/60 canonical splice variants (38.3%). Severe HGVS consequence therefore does not make a ClinVar P/LP assertion frequency-clean.

This pattern was systematic rather than a single-gene artifact. Severe-annotation discordance spanned 10 arrhythmia genes. After excluding CASQ2 and TRDN, the recessive/biallelic arrhythmia genes most likely to tolerate carrier frequency, 57/197 severe annotations (28.9%) remained frequency-discordant across 8 genes. Restricting to constrained genes did not eliminate the pattern: among genes with gnomAD LOEUF upper confidence bound <0.5, 24/76 severe annotations (31.6%) remained frequency-discordant. This creates a concrete clinical risk: a reviewer can see a frameshift, stop-gained, or canonical splice P/LP label and underweight a population frequency that is incompatible with the implied high-penetrance model.

Mechanism triage resolves the likely reviewer question: the 35.1% severe-discordance figure should not be read as "35.1% wrong." Of 92 severe-discordant assertions, 35 (38.0%) were in CASQ2 or TRDN, where recessive/biallelic architecture can make heterozygous carrier frequency biologically compatible with disease causality in the affected state. Among the remaining non-recessive severe-discordant assertions, only 1/57 was AC-supported, the KCNH2 splice case that enters the red-priority queue. Another 24/92 (26.1%) were in strongly constrained non-recessive genes but lacked AC support and therefore belong in surveillance rather than urgent downgrade review. The remaining 32/92 (34.8%) were architecture-unresolved or lower-constraint, low-AC tension signals. Thus, the biological claim is not an error-rate claim; it is that severe annotation creates a frequency-assertion tension universe that must be routed by mechanism, AC, inheritance, and review strength.

| Mechanism triage class | N (% of severe-discordant) | Genes | AC-supported | Interpretation |
|---|---:|---|---:|---|
| Carrier-compatible recessive/biallelic architecture | 35 (38.0%) | CASQ2, TRDN | 6 | Compatible with carrier frequency; not misclassification by itself |
| Non-recessive AC-supported high-penetrance tension | 1 (1.1%) | KCNH2 | 1 | Highest-priority mechanism adjudication |
| Non-recessive constrained-gene low-AC surveillance | 24 (26.1%) | ANK2, CACNA1C, SCN5A | 0 | Biologically uncomfortable but not urgent without AC support |
| Non-recessive other low-AC or architecture-unresolved surveillance | 32 (34.8%) | CACNB2, KCNE1, KCNH2, KCNJ2, KCNQ1 | 0 | Monitor or review routinely; do not infer downgrade from frequency alone |

The result is not that LOF variants are often benign. The narrower and more clinically useful conclusion is that LOF annotations are not sufficient to override population-frequency contradictions. When severe annotation, public P/LP assertion, and population frequency disagree, the correct review target is mechanism and effect size: recessive carrier state, founder enrichment, reduced penetrance, hypomorphic effect, transcript rescue or NMD escape, allele-specific evidence, or overcalling.

![Severe-annotation frequency discordance and expert-panel comparator](figures/vital_external_truth_biological_claim.png)

Expert-panel reviewed P/LP assertions in the cross-disease set provided an external curated comparator not based on future ClinVar churn. Among 204 expert-panel reviewed P/LP assertions, 39 (19.1%) had naive AF tension and 13 (6.4%) were AC-supported; 17/150 severe expert-panel assertions (11.3%) were frequency-discordant. VITAL did not designate any expert-panel record as red-priority (0/204), although three reached orange high-tension. This is the intended behavior: curated high-frequency exceptions should remain visible as tension cases without being converted into urgent downgrade calls. The comparator supports specificity of the red gate and reinforces the biological claim that frequency discordance can reflect mechanism-specific pathogenicity, founder or carrier architecture, low penetrance, or context-dependent effect rather than simple misclassification.
...
### Real reclassification impact and external curated truth check

The strict historical endpoint provides the closest public proxy to real clinical impact because it captures actual ClinVar category movement from P/LP to VUS, likely benign, or benign. In the 3,000-variant cross-disease historical set, 23 records underwent this strict P/LP-to-VUS/B/LB change by April 2026. VITAL-red captured 1/23 of these real reclassification events: CFAP91 VCV000812096, `NM_033364.4(CFAP91):c.124G>C (p.Asp42His)`, which moved from P/LP to VUS and had the exact profile VITAL is designed to prioritize: no assertion criteria, popmax AF=3.50e-4, qualifying AC=152, and VITAL=76.5. This is a real classification-level downstream event, not a proxy-review burden metric.

This result should be read with the same discipline as the rest of the framework. VITAL is not a sensitive detector of all future downgrades: 22/23 strict events were outside the red-priority queue. That low recall is clinically informative because most reclassifications arise from evidence streams other than population-frequency contradiction, such as phenotype specificity, segregation, functional reinterpretation, transcript effects, or expert-panel curation.

The external curated truth stress-test makes that boundary explicit. One strict future downgrade in the same 3,000-variant set had current expert-panel review status: RYR1 VCV001214001, `NM_000540.3(RYR1):c.14813T>C (p.Ile4938Thr)`, which moved to VUS. VITAL did not flag this record (VITAL=0.0; max frequency signal=8.99e-7; no AC-supported frequency tension). This is not a miss; it is the correct behavior for a frequency-assertion framework. A ClinGen/expert-panel downgrade without frequency tension belongs to expert curation and functional/phenotypic evidence review, not to a popmax/AC-driven re-review queue.

| Truth layer | Event count | VITAL-red captured | Interpretation |
|---|---:|---:|---|
| Real strict reclassification, P/LP to VUS/B/LB | 23/3,000 | 1 | VITAL captures rare frequency-driven downgrades, not all reclassifications |
| Frequency-observed strict reclassification | 13/732 | 1 | Even where frequency is observable, recall remains low by design |
| Expert-panel curated strict downgrade | 1/23 | 0 | Curated non-frequency downgrades are outside VITAL's intended scope |
...
### Restricted weight-consistency audit
...
The calibration audit was not used to search for better weights. It asked a narrower consistency question: once the analysis is restricted to variants with real frequency tension, do frequency-tension features contribute signal beyond review metadata? The answer was yes. Review-only probes achieved PR AUC 0.555 (95% bootstrap CI 0.482-0.645), while the combined review-plus-tension probe reached PR AUC 0.622 (95% CI 0.537-0.712). We report these values as audit statistics, not as evidence that VITAL should be refit as a predictive model.

Feature dominance supported the intended design. In the combined probe, standardized frequency-tension coefficients summed to 1.75 compared with 1.10 for review features (tension:review ratio 1.60). The strongest tension contributions were technical detectability, variant-type tension, gene constraint, and AC reliability, rather than raw AF magnitude alone. This is consistent with the prespecified weighting scheme, in which frequency-based and representation-aware components form the primary signal and review status acts as a modifier/gate rather than the main driver. Because the endpoint is proxy-based and sparse, the component weights were not fitted to outcome data by design.

![Restricted weight-consistency audit](figures/vital_cross_disease_3000_restricted_calibration_models.png)
...
The central finding is not that ClinVar P/LP variants are usually rare; that is expected. The stronger biological claim is that clinical genomics cannot treat severe annotation as a shield against population evidence. Frequency discordance was not a missense-only phenomenon: it appeared systematically among frameshift, stop-gained, and splice-labeled P/LP assertions across multiple genes, persisted after excluding canonical recessive arrhythmia genes, and remained visible in externally constrained genes. The 35.1% severe-discordance figure is not an error-rate estimate; mechanism triage shows that many cases are compatible with recessive carrier architecture or lack AC support for immediate action. The risk is subtler: a severe-looking ClinVar P/LP assertion can be both mechanistically plausible and too frequent for the high-penetrance label attached to it. LOF annotations are therefore not sufficient to override population-frequency contradictions; they identify a mechanism hypothesis that still has to be reconciled with ancestry, AC, penetrance, inheritance mode, transcript context, and review strength.

This is the problem VITAL operationalizes. Global AF detected only 13 arrhythmia variants above 1e-5, whereas popmax/global screening detected 115, meaning global-AF-only review missed 102/115 ancestry-aware frequency alerts. At the same time, more than 80% of variants lacked usable exact AF evidence, and indels/duplications were disproportionately affected. In this specific scenario, ACMG/AMP implementation is systematically failing: not because ACMG/AMP frequency criteria are wrong, but because common implementations can hide popmax tension, ignore AC support, or treat non-observation as rarity. BS1/BA1-style reasoning cannot work as intended if the workflow hides the frequency evidence it is supposed to evaluate.
...
The primary score is expert-specified by design rather than learned. Weight-profile, one-weight-at-a-time, and restricted weight-consistency audits showed that the queue is not driven by arbitrary single-weight choices, but they do not make VITAL a trained classifier or prove optimal weights. KCNH2 remained unstable in 3/5 alternative profiles, which is expected for threshold-adjacent cases and should be communicated as a review signal rather than a stable automated call. We did not fit component weights to outcome data because available endpoints are proxy-based, event counts are limited, and optimization would increase overfitting risk while reducing interpretability. Future disease-specific deployments should therefore recalibrate weights, AC gates, and thresholds against local disease architecture before prospective use. The workflow-concordance benchmark is also proxy-labeled and partly circular because weak review support and AC-supported frequency evidence help define both the benchmark positives and the red-priority gate; conventional diagnostic-performance summaries should therefore be read only as workflow-burden diagnostics, not clinical accuracy.

Historical audits are underpowered by design. With n=3 red-priority calls, even a true 20% strict-event rate would yield only 48.8% probability of observing at least one event; against a broader 6.8% background rate, detecting enrichment at alpha=0.05 would require at least 2/3 events and has only 10.4% power if the true red-event rate is 20%. Even in the 3,000-variant cross-disease sample, red-priority calls remained sparse and strict downgrade recall was low. The expanded instability endpoint includes soft database changes, including review-status or aggregate-text changes, and should be interpreted as curation activity rather than clinical downgrade. Public ClinVar snapshots also do not provide patient-level downstream effects such as diagnosis reversal, cascade-testing changes, surveillance changes, or therapy changes; the real-impact audit is therefore classification-level rather than patient-level. Release-to-release red-queue stability is based on only two freezes and should be tracked prospectively as retained, gained, and lost calls at each database update.

External panels are portability stress tests, not sensitivity analyses. The BRCA/MMR/APC comparator is descriptive rather than a clean negative control because cancer-predisposition genes have founder effects and incomplete penetrance. The expert-panel comparator is a curated stability layer, not an external gold standard for benignity. LOF subtype annotation was derived from ClinVar/HGVS strings and does not replace transcript-aware consequence annotation, NMD prediction, exon/domain context, or curated functional assays. Mechanism triage is likewise a gene-level routing heuristic, not proof that a variant is explained by founder effect, low penetrance, recessive architecture, or error. Therefore, the severe-annotation claim should be read narrowly: severe annotation is not sufficient to override population-frequency contradictions, not that LOF variants are commonly benign. The KCNH2 duplication result is statistical rather than mechanistic and is not used as a clinical recommendation; duplications account for much of the non-overlap signal, but the underlying cause remains unresolved. Finally, the data freeze was April 21, 2026; all red-priority variants should be rechecked against live ClinVar before publication, clinical reporting, or patient-facing decisions.

## Clinical and research implications

These results support several practical recommendations for clinical genomics workflows:

1. Stop relying on global AF alone for database auditing. In this arrhythmia audit, global-AF-only screening missed 102/115 ancestry-aware frequency alerts, creating an avoidable misinterpretation hazard for P/LP assertions.

2. Use ancestry-aware frequency thresholds. Popmax identifies frequency tension that global AF misses and should be part of any ACMG-style BS1/BA1 review workflow.

3. Require AC support before acting on frequency contradictions. AC >=20 is not the only possible threshold, but it prevents overreaction to one- or two-allele observations.

4. Do not treat severe annotation as self-validating. Frameshift, stop-gained, and splice labels should trigger mechanism review, but they do not cancel population-frequency contradictions.

5. Do not treat absence as rarity for all variant types. Indels and duplications require special caution because exact gnomAD representation is systematically lower.

6. Separate review prioritization from clinical classification. Red-priority should initiate expert re-review, not automatically assign benignity.

7. Add inheritance-aware routing to the review queue. Dominant/high-penetrance calls, recessive carrier-frequency signals, founder or low-penetrance alleles, pharmacogenomic susceptibility assertions, and unknown-mechanism variants should not be interpreted through the same frequency lens. VITAL identifies tension; the review workflow should route that tension through the appropriate disease-mechanism pathway.

8. Report explainable component scores. AF pressure, AC reliability, detectability, gene context, and review fragility should be visible for each variant.

9. Use external stress tests and historical snapshots. Frameworks that audit clinical databases should show that they behave conservatively outside the development context and, where possible, show preliminary enrichment for future instability with uncertainty reported explicitly.

10. Track review burden as a clinical endpoint. A framework that removes false-positive re-evaluation calls can improve laboratory throughput, shorten review queues, and focus expert decision-making on variants most likely to change patient-facing interpretation.
...
Key output groups include per-variant VITAL scores and component explanations, method-comparison tables, threshold and AC sensitivity sweeps, weight-sensitivity tables, semi-empirical calibration outputs, historical validation outputs, external-panel stress tests, 3,000-variant cross-disease validation outputs, ClinVar time-series trajectory outputs, severe-annotation discordance audits, expert-panel comparator outputs, real reclassification truth audits, and generated figures. The most central files are `arrhythmia_vital_scores.csv`, `arrhythmia_vital_component_breakdown.csv`, `arrhythmia_vital_method_comparison.csv`, `arrhythmia_vital_pathogenicity_tension_continuum.csv`, `arrhythmia_vital_one_weight_sensitivity.csv`, `vital_severe_annotation_frequency_discordance_summary.csv`, `vital_severe_annotation_gene_spread.csv`, `vital_severe_annotation_mechanism_triage.csv`, `vital_expert_panel_truth_validation.csv`, `vital_external_truth_claim_tests.csv`, `vital_real_reclassification_clinical_impact_audit.csv`, `vital_real_reclassification_case_examples.csv`, `vital_cross_disease_3000_restricted_calibration_model_metrics.csv`, `vital_cross_disease_3000_restricted_calibration_feature_dominance.csv`, `vital_red_manual_review_live_check.csv`, `vital_cross_disease_3000_cross_disease_validation_summary.csv`, `vital_cross_disease_3000_2023_01_to_current_vital_historical_enrichment.csv`, `vital_cross_disease_3000_clinvar_time_series_wide.csv`, `arrhythmia_current_red_clinvar_time_series_red_queue.csv`, and `arrhythmia_intermediate_only_2024_2025_vital_scores.csv`.

The biological contrast layer is provided as `data/processed/arrhythmia_gene_biological_context.csv`, `data/processed/arrhythmia_vital_biological_contrast_cases.csv`, `data/processed/arrhythmia_vital_biological_contrast_summary.csv`, and `figures/vital_biological_contrast_cases.png`. The severe-annotation and curated-comparator extension is provided as `data/processed/vital_severe_annotation_frequency_discordance_summary.csv`, `data/processed/vital_severe_annotation_gene_spread.csv`, `data/processed/vital_severe_annotation_mechanism_triage.csv`, `data/processed/vital_expert_panel_truth_validation.csv`, `data/processed/vital_expert_panel_high_tension_examples.csv`, `data/processed/vital_external_truth_claim_tests.csv`, `figures/vital_external_truth_biological_claim.png`, and `supplementary_tables/Supplementary_Table_S22_external_truth_biological_claim.tsv`. These files are generated by `src/run_vital_biological_contrast.py` and `src/run_vital_external_truth_claim.py` from gnomAD v4.1 constraint metrics, Human Protein Atlas v23 tissue-consensus expression downloads, VITAL score tables, and the 3,000-variant cross-disease comparator; raw external downloads are treated as reproducible inputs and are not committed as bulky repository artifacts.
...
- Supplementary Table S21: restricted VITAL weight-consistency audit, including review-only, tension-only, and combined diagnostic probes.
- Supplementary Table S22: severe-annotation frequency discordance and expert-panel comparator, including LOF subtype summaries, recessive-gene exclusion, constrained-gene checks, mechanism triage, and curated high-tension examples.
- Supplementary Table S23: real strict reclassification and external curated truth audit, including the CFAP91 captured downgrade and the RYR1 expert-panel downgrade outside VITAL's frequency-tension scope.
