...
| Reconciliation layer | Count | Interpretation |
|---|---:|---|
| Strict exact/equivalent evidence | 357 | Exact match plus trim/decomposition-aware equivalent recovery |
| Locus/regional context only | 1326 | Same-locus or nearby variation observed, but no usable exact/equivalent AF |
| Still unevaluable after reconciliation | 48 | No exact, equivalent, or local contextual recovery |
...
### Frequency tension concentrates in action-linked clinical contexts
...
| Clinical context | Frequency-tension variants | Weak review | AC-supported | Red |
|---|---:|---:|---:|---:|
| Guideline-driven/cascade core | 103/115 | 51 | 9 | 3 |
| Drug-restriction context | 60/115 | 31 | 2 | 2 |
| Device/intensive-surveillance context | 59/115 | 28 | 2 | 2 |
...
### Clinical decision projection shows one label can imply incompatible pathways

The practical consequence of label-state collapse is that one public P/LP label can support two incompatible clinical pathways depending on how it is read. We therefore projected, without assuming any real patient-level downstream event, what follows if each red-priority label is read as unqualified high-penetrance Mendelian disease versus as the mechanism-aware state required by the frequency and evidence context.

| Case | If read as generic Mendelian P/LP | If read as state-aware assertion | Guideline-facing tension |
|---|---|---|---|
| SCN5A VCV000440850 | Can support Brugada diagnostic closure, broad cascade testing, arrhythmia surveillance, drug-avoidance counseling, and phenotype-triggered ICD/risk-stratification discussions | Does not establish monogenic Brugada diagnosis by itself; cascade testing should be limited or qualified as susceptibility/haplotype tracking, and management should remain phenotype/context driven | ClinVar terminology allows low-penetrance and risk-allele states, but the public record remains generic Pathogenic with no assertion criteria |
| TRDN VCV001325231 | Can be misread as CPVT5 disease causality and dominant-risk family counseling if phase and second-allele status are ignored | Should be routed as carrier-compatible or biallelic affected-state evidence; family testing should prioritize phase and second-allele clarification | Generic P/LP does not encode carrier versus affected-state use |
| KCNH2 VCV004535537 | Can support Long QT diagnostic closure, familial testing, QT surveillance, and medication-avoidance decisions if accepted as unqualified LP | Requires splice/same-site/segregation adjudication; cascade and management decisions should remain provisional and phenotype-linked | Single-submitter LP plus AC-supported frequency tension creates an expert-review need, but the public label does not expose splice certainty or penetrance state |
...
6. Run a clinical decision projection for red-priority variants before acting: ask what would follow if the label were treated as Mendelian disease, and what would follow if it were susceptibility, carrier-state, or mechanism-unresolved evidence.

7. Use the label-state ontology as a transition map: move from generic P/LP to context-qualified assertion, carrier assertion, penetrance-qualified assertion, urgent re-review, or gray representation-uncertain queue as evidence dictates.

8. Separate re-review prioritization from final classification. VITAL tells laboratories what to inspect first, not what to believe automatically.
